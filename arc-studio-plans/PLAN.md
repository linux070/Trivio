...
5. `src/components/LandingPage.tsx` — full hero landing page: "TRIVIO" wordmark, "having fun onchain" tagline, game explainer (how it works, categories, prize model), email sign-in input + OTP flow, and ConnectKit wallet connect button — both auth paths render here, no separate screen
...
3. Build `LandingPage.tsx` — TRIVIO hero, tagline, game explainer, email OTP input, and ConnectKit button all on one page
...
- [ ] Landing page shows "TRIVIO" wordmark, "having fun onchain" tagline, game explainer, email sign-in, and ConnectKit wallet button
